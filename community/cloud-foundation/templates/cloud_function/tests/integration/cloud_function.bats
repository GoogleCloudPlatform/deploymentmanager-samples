...
@test "Verifying cloud functions were created in deployment ${DEPLOYMENT_NAME}" {
...
@test "Verifying test-function-https-name-${RAND} properties are set" {
...
@test "Verifying test-function-https-name-${RAND} trigger is set" {
    run gcloud functions describe test-function-https-name-${RAND} \
        --project "${CLOUD_FOUNDATION_PROJECT_ID}"
    [[ "$output" =~ "url: https://us-central1-${CLOUD_FOUNDATION_PROJECT_ID}.cloudfunctions.net/test-function-https-name-${RAND}" ]]
}

@test "Verifying test-function-topic-${RAND} trigger is set" {
...
@test "Verifying test-function-storage-${RAND} trigger is set" {

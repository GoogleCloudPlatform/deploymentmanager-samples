...
RANDOM_FILE="/tmp/${FAAS_ORGANIZATION_ID}-vpn.txt"
...
if [[ -e "${RANDOM_FILE}" ]]; then
    ...
    DEPLOYMENT_NAME="${FAAS_PROJECT_ID}-vpn-${RAND}"
...
function create_config() {
    ...
    envsubst < tests/fixtures/configs/vpn.yaml > "${CONFIG}"
}
...
@test "Creating deployment ${DEPLOYMENT_NAME} from ${CONFIG}" {
    gcloud deployment-manager deployments create "${DEPLOYMENT_NAME}" \
        --project "${FAAS_PROJECT_ID}" --config "${CONFIG}"
}
...
@test "Verifying the forwarding rules in deployment ${DEPLOYMENT_NAME}" {
    ...
    run gcloud compute forwarding-rules list --project "${FAAS_PROJECT_ID}"
    [ "$status" -eq 0 ]

...
RANDOM_FILE="/tmp/${ORGANIZATION_ID}-network.txt"
...
if [[ -e ${RANDOM_FILE} ]]; then
    ...
    DEPLOYMENT_NAME="${PROJECT_NAME}-network"
...
@test "Creating deployment ${DEPLOYMENT_NAME} from ${CONFIG}" {
    run gcloud deployment-manager deployments create ${DEPLOYMENT_NAME} --config ${CONFIG} --project ${PROJECT_NAME}
}

@test "Verifying resources were created in deployment ${DEPLOYMENT_NAME}" {
    run gcloud compute networks list --filter="name:test-network-${RAND}" --project ${PROJECT_NAME}
    [[ "$output" =~ "test-network-${RAND}" ]]
}

@test "Verifying subnets were created in deployment ${DEPLOYMENT_NAME}" {
    run gcloud compute networks subnets list --project ${PROJECT_NAME}
    [[ "$output" =~ "test-subnetwork-${RAND}-1" ]]
    [[ "$output" =~ "test-subnetwork-${RAND}-2" ]]
}

@test "Deployment Delete" {
    run gcloud deployment-manager deployments delete ${DEPLOYMENT_NAME} -q --project ${PROJECT_NAME}
}

...
EXTERNAL_IP  = @COMPUTE_PUBLIC_IPS@

DISK_SIZE_GB = '@DISK_SIZE_GB@'
DISK_TYPE    = '@DISK_TYPE@'

NETWORK_TYPE = 'subnetwork'
NETWORK      = "projects/%s/regions/%s/subnetworks/pbs-subnet" % (PROJECT, REGION)

SCONTROL     = '/apps/pbs/current/bin/scontrol'
LOGFILE      = '/apps/pbs/log/resume.log'
...
def create_instance(compute, project, zone, instance_type, instance_name):
  ...
  startup_script = open('/apps/pbs/scripts/startup-script.py', 'r').read()
...
def main(short_node_list):
  ...
  for node_name in operations:
    try:
      ...
      logging.exception("Error in adding %s to pbs (%s)" % (node_name, str(e)))
